...
| 2 | Trading setup accelerators | Supplier-product-to-buyer wizard (**done**), recent-load widgets (**done**), buyer/supplier tracking helpers | Relationship/product setup can be done with the same few-click paths as legacy where still useful. |
...
| 6 | Low-priority helpers | Location favorite toggle (**done**), load export (**done**), favorite list filters, quick status/substatus helpers | Implement only if users confirm the workflow still matters. |
